...
${PY_MCU_CM} -d ${SERIAL_DEVICE} -v ${VERBOSITY} -c clk_setup -p IC1 IC1_40.079IN1_40.079_IN0,1,2,3,4,5,6,7,8,FB-Registers.txt

echo "Program the clock synthesizer chip IC2 (Si5345A) for SL communication."
${PY_MCU_CM} -d ${SERIAL_DEVICE} -v ${VERBOSITY} -c clk_setup -p IC2 IC2,3,6,7_40.079IN0_240.474_FB-Registers.txt

echo "Program the clock synthesizer chip IC4 (Si5345A) for SL communication."
${PY_MCU_CM} -d ${SERIAL_DEVICE} -v ${VERBOSITY} -c clk_setup -p IC4 IC2,3,6,7_40.079IN0_240.474_FB-Registers.txt

echo "Program the clock synthesizer chip IC6 (Si5345A) for SL communication."
${PY_MCU_CM} -d ${SERIAL_DEVICE} -v ${VERBOSITY} -c clk_setup -p IC6 IC2,3,6,7_40.079IN0_240.474_FB-Registers.txt

echo "Program the clock synthesizer chip IC8 (Si5345A) for SL communication."
${PY_MCU_CM} -d ${SERIAL_DEVICE} -v ${VERBOSITY} -c clk_setup -p IC8 IC2,3,6,7_40.079IN0_240.474_FB-Registers.txt

echo "Program the clock synthesizer chip IC10 (SI5345A) to generate a fixed 40 MHz clock from the 120MHz rec-clock from SL."
${PY_MCU_CM} -d ${SERIAL_DEVICE} -v ${VERBOSITY} -c clk_setup -p IC10 IC10_120.237IN0_40.079OUT0-1-2-3.txt
